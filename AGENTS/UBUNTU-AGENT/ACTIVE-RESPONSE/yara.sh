...
then
    echo "Sentinel AI: ERROR - YARA active response error. YARA path and rules parameters are mandatory." >> ${LOG_FILE}
    exit 1
fi
#------------------------- Main workflow --------------------------#
...
then
    ...
    if rm -rf "$FILENAME"; then
        echo "Sentinel AI: INFO - Successfully deleted $FILENAME" >> ${LOG_FILE}
    else
        echo "Sentinel AI: INFO - Unable to delete $FILENAME" >> ${LOG_FILE}
    fi
    ...
    while read -r line; do
        ...
        if [[ $description != "" ]]; then
            ...
            if [[ $? -ne 0 ]]; then
                ...
                echo "Sentinel AI: ERROR - Invalid LLM API request" >> ${LOG_FILE}
                # Log Yara scan result without LLM response
                echo "Sentinel AI: INFO - Scan result: $line | AI_response: none" >> ${LOG_FILE}
            else
                # Check if the response text is empty and handle the error
                if [[ -z "$llm_response" ]]; then
                    echo "Sentinel AI: ERROR - LLM API returned empty response" >> ${LOG_FILE}
                else
                    # Combine the YARA scan output and LLM response
                    combined_output="Sentinel AI: INFO - Scan result: $line | AI_response: $llm_response"
                    # Append the combined output to the log file
                    echo "$combined_output" >> ${LOG_FILE}
                fi
            fi
        else
            echo "Sentinel AI: INFO - Scan result: $line" >> ${LOG_FILE}
        fi
    done <<< "$YARA_output"
    # If API request was invalid, log a specific message
    if $api_request_invalid; then
        echo "Sentinel AI: INFO - API request is invalid. LLM response omitted." >> ${LOG_FILE}
    fi
else
    echo "Sentinel AI: INFO - No YARA rule matched." >> ${LOG_FILE}
fi
exit 0;
